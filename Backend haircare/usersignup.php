...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($name) || empty($age) || empty($gender) || empty($number) || empty($email) || empty($password)) {
        echo json_encode([
            'status' => false,
    ...
    try {
        ...
        $stmt = $conn->prepare("INSERT INTO usersignup (Name, Age, Gender, Number, Email, Password) VALUES (?, ?, ?, ?, ?, ?)");
        ...
        // Bind the parameters
        $stmt->bind_param("sissss", $name, $age, $gender, $number, $email, $hashedPassword);
